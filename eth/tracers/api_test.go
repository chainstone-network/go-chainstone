...
func TestTraceCall(t *testing.T) {
	...
	genesis := &core.Genesis{Alloc: core.GenesisAlloc{
		accounts[0].addr: {Balance: big.NewInt(params.Chainstone)},
		accounts[1].addr: {Balance: big.NewInt(params.Chainstone)},
		accounts[2].addr: {Balance: big.NewInt(params.Chainstone)},
	}}
	genBlocks := 10
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &ethapi.StateOverride{
					randomAccounts[0].addr: ethapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Chainstone)))},
				},
			},
