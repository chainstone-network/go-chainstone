...
// Package filters implements an chainstone filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  chainstone.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit chainstone.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit chainstone.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit chainstone.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit chainstone.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
