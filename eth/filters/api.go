...
// information related to the Chainstoneeum protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(chainstone.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as chainstone.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria chainstone.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(chainstone.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
