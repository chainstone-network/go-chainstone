...
// support all the operations needed by the Chainstoneeum chain protocols.
...
// newHandler returns a handler for all Chainstoneeum chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	// Execute the Chainstoneeum handshake
	var (
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Chainstoneeum handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Chainstoneeum peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Chainstoneeum peer registration failed", "err", err)
		return err
	}
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Chainstoneeum peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Chainstoneeum peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Chainstoneeum peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("Chainstoneeum protocol stopped")
}
