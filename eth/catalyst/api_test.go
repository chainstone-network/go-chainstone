...
var (
	...
	// testAddr is the Chainstoneeum address of the tester account.
...
func startEthService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *eth.Chainstoneeum) {
	t.Helper()
	...
	ethservice.SetChainstonebase(testAddr)
...
func setupBlocks(t *testing.T, ethservice *eth.Chainstoneeum, n int, parent *types.Block, callback func(parent *types.Block)) {
	api := NewConsensusAPI(ethservice)
	for i := 0; i < n; i++ {
		callback(parent)
