...
func Register(stack *node.Node, backend *eth.Chainstoneeum) error {
...
type ConsensusAPI struct {
	...
	//     persist any bad block information to disk as a bug in Gchain could end
	//     up blocking a valid chain, even if a later Gchain update would accept
	//     it.
	//   - Bad blocks will get forgotten after a certain threshold of import
	//     attempts and will be retried. The rationale is that if the network
	//     really-really-really tries to feed us a block, we should give it a
	//     new chance, perhaps us being racey instead of the block being legit
	//     bad (this happened in Gchain at a point with import vs. pending race).
	...
	// Gchain can appear to be stuck or do strange things if the beacon client is
...
func NewConsensusAPI(eth *eth.Chainstoneeum) *ConsensusAPI {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config beacon.TransitionConfigurationV1) (*beacon.TransitionConfigurationV1, error) {
	...
	if ttd == nil || ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "gchain", ttd, "beacon", config.TerminalTotalDifficulty)
