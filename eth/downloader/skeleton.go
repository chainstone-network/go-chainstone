// Copyright 2022 The go-chainstone Authors
...
// the process. This is usually the case when Gchain is shutting down and some events
...
// Chainstoneeum block sync protocol.
//
...
//
// Opposed to the original Chainstoneeum block synchronization which is trustless (and
...
// not from within (vs. legacy Chainstoneeum sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gchain is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gchain requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
