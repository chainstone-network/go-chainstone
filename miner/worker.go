...
// setChainstonebase sets the chainstonebase used to initialize the block coinbase field.
func (w *worker) setChainstonebase(addr common.Address) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.coinbase = addr
}
...
func (w *worker) commitWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if w.isRunning() {
		if w.coinbase == (common.Address{}) {
			log.Error("Refusing to mine without chainstonebase")
			return
		}
...
func (w *worker) commit(env *environment, interval func(), update bool, start time.Time) error {
	if w.isRunning() {
		...
		// https://github.com/chainstone-network/go-chainstone/issues/24299
		env := env.copy()
...
func totalFees(block *types.Block, receipts []*types.Receipt) *big.Float {
	...
	return new(big.Float).Quo(new(big.Float).SetInt(feesWei), new(big.Float).SetInt(big.NewInt(params.Chainstone)))
}
