...
// Package miner implements Chainstoneeum block creation and mining.
package miner
...
type Config struct {
	Chainstonebase  common.Address `toml:",omitempty"` // Public address for block mining rewards (default = first account)
	...
	for {
		...
		case ev := <-dlEventCh:
			...
			case downloader.FailedEvent:
				canStart = true
				if shouldStart {
					miner.SetChainstonebase(miner.coinbase)
					miner.worker.start()
				}
...
func (miner *Miner) SetChainstonebase(addr common.Address) {
	miner.coinbase = addr
	miner.worker.setChainstonebase(addr)
}
