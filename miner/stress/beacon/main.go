...
type ethNode struct {
	...
	ethBackend *eth.Chainstoneeum
	lapi       *lescatalyst.ConsensusAPI
	lesBackend *les.LightChainstoneeum
}
...
func makeFullNode(genesis *core.Genesis) (*node.Node, *eth.Chainstoneeum, *ethcatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Chainstoneeum node
	...
	// Create the node and configure a full Chainstoneeum node on it
...
func makeLightNode(genesis *core.Genesis) (*node.Node, *les.LightChainstoneeum, *lescatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Chainstoneeum node
	datadir, _ := os.MkdirTemp("", "")
