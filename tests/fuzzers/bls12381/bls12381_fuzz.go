// Copyright 2021 The go-chainstone Authors
...
func FuzzCrossPairing(data []byte) int {
	...
	if !(bytes.Equal(cResult.Marshal(), bls12381.NewGT().ToBytes(kResult))) {
		panic("pairing mismatch gnark / gchain ")
	}
	...
	if !(bytes.Equal(res, bls12381.NewGT().ToBytes(kResult))) {
		panic("pairing mismatch blst / gchain")
	}
...
func FuzzCrossG1Add(data []byte) int {
	...
	if !(bytes.Equal(cp.Marshal(), g1.ToBytes(&kp))) {
		panic("G1 point addition mismatch gnark / gchain ")
	}

	bl3 := blst.P1AffinesAdd([]*blst.P1Affine{bl1, bl2})
	if !(bytes.Equal(cp.Marshal(), bl3.Serialize())) {
		panic("G1 point addition mismatch blst / gchain ")
	}
...
func FuzzCrossG2Add(data []byte) int {
	...
	if !(bytes.Equal(cp.Marshal(), g2.ToBytes(&kp))) {
		panic("G2 point addition mismatch gnark / gchain ")
	}
...
func FuzzCrossG1MultiExp(data []byte) int {
	...
	for i := 0; i < 17; i++ {
		// note that gchain/crypto/bls12381 works only with scalars <= 32bytes
		...
		gchainScalars = append(gchainScalars, s)
		var gnarkScalar = &fr.Element{}
		gnarkScalar = gnarkScalar.SetBigInt(s).FromMont()
		gnarkScalars = append(gnarkScalars, *gnarkScalar)

		gchainPoints = append(gchainPoints, new(bls12381.PointG1).Set(kp1))
		gnarkPoints = append(gnarkPoints, *cp1)
	}
	if len(gchainScalars) == 0 {
		return 0
	}
	// compute multi exponentiation
	g1 := bls12381.NewG1()
	kp := bls12381.PointG1{}
	if _, err := g1.MultiExp(&kp, gchainPoints, gchainScalars); err != nil {
		panic(fmt.Sprintf("G1 multi exponentiation errored (gchain): %v", err))
	}
	// note that gchain/crypto/bls12381.MultiExp mutates the scalars slice (and sets all the scalars to zero)
	...
	// compare result
	if !(bytes.Equal(cp.Marshal(), g1.ToBytes(&kp))) {
		panic("G1 multi exponentiation mismatch gnark / gchain ")
	}
...
func getG1Points(input io.Reader) (*bls12381.PointG1, *gnark.G1Affine, *blst.P1Affine, error) {
	...
	// marshal gnark point -> gchain point
	g1 := bls12381.NewG1()
	kp, err := g1.FromBytes(cpBytes)
	if err != nil {
		panic(fmt.Sprintf("Could not marshal gnark.G1 -> gchain.G1: %v", err))
	}
	if !bytes.Equal(g1.ToBytes(kp), cpBytes) {
		panic("bytes(gnark.G1) != bytes(gchain.G1)")
	}
	...
	if !bytes.Equal(p1.Serialize(), cpBytes) {
		panic("bytes(blst.G1) != bytes(gchain.G1)")
	}
...
func getG2Points(input io.Reader) (*bls12381.PointG2, *gnark.G2Affine, *blst.P2Affine, error) {
	...
	if err != nil {
		panic(fmt.Sprintf("Could not marshal gnark.G2 -> gchain.G2: %v", err))
	}
	if !bytes.Equal(g2.ToBytes(kp), cpBytes) {
		panic("bytes(gnark.G2) != bytes(gchain.G2)")
	}
	...
	if !bytes.Equal(p2.Serialize(), cpBytes) {
		panic("bytes(blst.G2) != bytes(gchain.G2)")
	}
