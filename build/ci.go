...
var (
	// Files that end up in the gchain*.zip archive.
	gchainArchiveFiles = []string{
		"COPYING",
		executablePath("gchain"),
	}

	// Files that end up in the gchain-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Chainstoneeum contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Chainstoneeum bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Chainstoneeum Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gchain",
			Description: "Chainstoneeum CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "Chainstoneeum private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Chainstoneeum account management tool.",
		},
	}

	// A debian package is created for all executables listed here.
	debChainstoneeum = debPackage{
		Name:        "chainstone",
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "gchainstore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	for _, archive := range []string{gchain, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gchain-release.pub"
...
func doDocker(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the docker image (usually "chainstone/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - chainstone/client-go:latest                            - Pushes to the master branch, Gchain only
	//  - chainstone/client-go:stable                            - Version tag publish on GitHub, Gchain only
	//  - chainstone/client-go:alltools-latest                   - Pushes to the master branch, Gchain & tools
	//  - chainstone/client-go:alltools-stable                   - Version tag publish on GitHub, Gchain & tools
	//  - chainstone/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Gchain only
	//  - chainstone/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Gchain & tools
	//  - chainstone/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Gchain only
	//  - chainstone/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Gchain & tools
	var tags []string
	...
	if *image {
		...
		for _, tag := range tags {
			gchainImage := fmt.Sprintf("%s:%s-%s", *upload, tag, runtime.GOARCH)
			...
			// master branch.
			for _, img := range []string{gchainImage, toolImage} {
			...
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:TAG", *upload), gchainImage)
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:alltools-TAG", *upload), toolImage)
			build.MustRunCommand("docker", "push", gchainImage)
	...
	if len(*manifest) != 0 {
		...
		for i := 0; i < 2; i++ { // 2 attempts, second is race check
			...
			for _, tag := range tags {
				for _, arch := range strings.Split(*manifest, ",") {
					gchainImage := fmt.Sprintf("%s:%s-%s", *upload, tag, arch)
		...
		// Assemble and push the Gchain manifest image
		for _, tag := range tags {
			gchainImage := fmt.Sprintf("%s:%s", *upload, tag)

			var gchainSubImages []string
			for _, arch := range strings.Split(*manifest, ",") {
				gchainSubImages = append(gchainSubImages, gchainImage+"-"+arch)
			}
			build.MustRunCommand("docker", append([]string{"manifest", "create", gchainImage}, gchainSubImages...)...)
			build.MustRunCommand("docker", "manifest", "push", gchainImage)
		}
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "chainstone/chainstone")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gchain-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-chainstone sources.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gchain-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "chainstone"
...
type debMetadata struct {
	...
	// go-chainstone version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Chainstoneeum Builds <fjl@chainstone.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gchain.exe" {
			gchainTool = file
		} else {
	...
	installer, _ := filepath.Abs("gchain-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gchain.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "gchainstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	tc := new(build.GoToolchain)
	...
	if *local {
		...
		os.Rename("gchain.aar", filepath.Join(GOBIN, "gchain.aar"))
		os.Rename("gchain-sources.jar", filepath.Join(GOBIN, "gchain-sources.jar"))
		return
	}
	...
	archive := "gchain-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gchain.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "gchain-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Gchain.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Gchain.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "gchainstore/builds")`)
