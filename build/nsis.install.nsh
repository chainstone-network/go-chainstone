Name "gchain ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install gchain binary
Section "Gchain" GCHAIN_IDX
  SetOutPath $INSTDIR
  file {{.Gchain}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\gchain.exe"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\gchain.exe" "attach"
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe"

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "Gchain incoming peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Gchain outgoing peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Gchain UDP discovery (UDP:30303)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "Gchain incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\gchain.exe" "" "" "Chainstoneeum" 30303 "" "" ""
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${GCHAIN_IDX}
    SectionGetSize ${GCHAIN_IDX} $0
