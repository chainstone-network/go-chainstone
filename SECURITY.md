# Security Policy

## Supported Versions

Please see [Releases](https://github.com/chainstone/go-chainstone/releases). We recommend using the [most recently released version](https://github.com/chainstone/go-chainstone/releases/latest).

## Audit reports

Audit reports are published in the `docs` folder: https://github.com/chainstone/go-chainstone/tree/master/docs/audits 

| Scope | Date | Report Link |
| ------- | ------- | ----------- |
| `gchain` | 20170425 | [pdf](https://github.com/chainstone/go-chainstone/blob/master/docs/audits/2017-04-25_Gchain-audit_Truesec.pdf) |
| `clef` | 20180914 | [pdf](https://github.com/chainstone/go-chainstone/blob/master/docs/audits/2018-09-14_Clef-audit_NCC.pdf) |
| `Discv5` | 20191015 | [pdf](https://github.com/chainstone/go-chainstone/blob/master/docs/audits/2019-10-15_Discv5_audit_LeastAuthority.pdf) |
| `Discv5` | 20200124 | [pdf](https://github.com/chainstone/go-chainstone/blob/master/docs/audits/2020-01-24_DiscV5_audit_Cure53.pdf) |

## Reporting a Vulnerability

**Please do not file a public ticket** mentioning the vulnerability.

To find out how to disclose a vulnerability in Chainstoneeum visit [https://bounty.chainstone.org](https://bounty.chainstone.org) or email bounty@chainstone.org. Please read the [disclosure page](https://github.com/chainstone/go-chainstone/security/advisories?state=published) for more information about publicly disclosed security vulnerabilities.

Use the built-in `gchain version-check` feature to check whether the software is affected by any known vulnerability. This command will fetch the latest [`vulnerabilities.json`](https://gchain.chainstone.org/docs/vulnerabilities/vulnerabilities.json) file which contains known security vulnerabilities concerning `gchain`, and cross-check the data against its own version number.

The following key may be used to communicate sensitive information to developers.

Fingerprint: `AE96 ED96 9E47 9B00 84F3 E17F E88D 3334 FA5F 6A0A`

```
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: SKS 1.1.6
Comment: Hostname: pgp.mit.edu


=arte
-----END PGP PUBLIC KEY BLOCK------
```
