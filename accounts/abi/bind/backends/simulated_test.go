...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != chainstone.NotFound {
		t.Fatalf("err should be `chainstone.NotFound` but received %v", err)
	}
...
func TestEstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.Chainstone)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		name        string
		message     chainstone.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", chainstone.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", chainstone.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestEstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.Chainstone*2 + 2e17)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", chainstone.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", chainstone.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", chainstone.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(1e14), // gascost = 2.1chainstone
			Value:    big.NewInt(1e17), // the remaining balance for fee is 2.1chainstone
			Data:     nil,
		}, 21000, nil},

		{"EstimateWithSuperhighPrice", chainstone.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(2e14), // gascost = 4.2chainstone
			Value:    big.NewInt(100000000000),
			Data:     nil,
		}, 21000, errors.New("gas required exceeds allowance (10999)")}, // 10999=(2.2chainstone-1000wei)/(2e14)

		{"EstimateEIP1559WithHighFees", chainstone.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
			GasFeeCap: big.NewInt(1e14), // maxgascost = 2.1chainstone
			GasTipCap: big.NewInt(1),
			Value:     big.NewInt(1e17), // the remaining balance for fee is 2.1chainstone
			Data:      nil,
		}, params.TxGas, nil},

		{"EstimateEIP1559WithSuperHighFees", chainstone.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
			GasFeeCap: big.NewInt(1e14), // maxgascost = 2.1chainstone
			GasTipCap: big.NewInt(1),
			Value:     big.NewInt(1e17 + 1), // the remaining balance for fee is 2.1chainstone
			Data:      nil,
		}, params.TxGas, errors.New("gas required exceeds allowance (20999)")}, // 20999=(2.2chainstone-0.1chainstone-1wei)/(1e14)
	}
	for i, c := range cases {
		got, err := sim.EstimateGas(context.Background(), c.message)
		if c.expectError != nil {
...
func TestPendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, chainstone.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, chainstone.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestCallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, chainstone.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, chainstone.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
