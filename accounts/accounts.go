...
// Package accounts implements high level Chainstoneeum account management.
package accounts
...
// Account represents an Chainstoneeum account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Chainstoneeum account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain chainstone.ChainStateReader)
	...
	// by the Chainstoneeum prefix scheme
	// It looks up the account specified either solely via its address contained within,
	// or optionally with the aid of any location metadata from the embedded URL field.
	//
	// If the wallet requires additional authentication to sign the request (e.g.
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Chainstoneeum Signed Message:\n%d%s", len(data), string(data))
