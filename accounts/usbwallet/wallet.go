...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Chainstoneeum
...
type wallet struct {
	...
	deriveChain     chainstone.ChainStateReader // Blockchain state reader to discover used account with
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Chainstoneeum account
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain chainstone.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
