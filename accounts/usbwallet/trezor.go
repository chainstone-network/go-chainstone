...
import (
	...
	"github.com/chainstone-network/go-chainstone/accounts/usbwallet/trezor"
...
// or whether the Chainstoneeum app was not started on it.
...
// Chainstoneeum address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.ChainstoneeumAddress)
	if _, err := w.trezorExchange(&trezor.ChainstoneeumGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.ChainstoneeumSignTx{
	...
	response := new(trezor.ChainstoneeumTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.ChainstoneeumTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
