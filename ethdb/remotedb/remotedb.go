...
// Package remotedb implements the key-value database layer based on a remote gchain
// node. Under the hood, it utilises the `debug_dbGet` method to implement a
// read-only database.
// There really are no guarantees in this database, since the local gchain does not
