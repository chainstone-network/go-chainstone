...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req               *SignDataRequest
		useChainstoneeumV = true // Default to use V = 27 or 28, the legacy Chainstoneeum format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useChainstoneeumV, err
	}
	...
	case apitypes.ApplicationClique.Mime:
		// Clique is the Chainstoneeum PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useChainstoneeumV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationClique.Mime)
		}
		...
		useChainstoneeumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: cliqueRlp, Messages: messages, Hash: sighash}
	default: // also case TextPlain.Mime:
		// Calculates an Chainstoneeum ECDSA signature for:
		// hash = keccak256("\x19Chainstoneeum Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useChainstoneeumV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
	...
	return req, useChainstoneeumV, nil
}
