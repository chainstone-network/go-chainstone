...
import (
	...
	"github.com/chainstone-network/go-chainstone/les/checkpointoracle"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some chainstone to user1
		...
		case 1:
			...
			// bankUser transfers more chainstone to user1
			...
			// user1 relays chainstone to user2
		...
		case 2:
			...
			// bankUser transfer some chainstone to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
