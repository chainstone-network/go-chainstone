...
// Package les implements the Light Chainstoneeum Subprotocol.
package les
...
import (
	...
	vfc "github.com/chainstone-network/go-chainstone/les/vflux/client"
...
type LightChainstoneeum struct {
	lesCommons
...
func New(stack *node.Node, config *ethconfig.Config) (*LightChainstoneeum, error) {
...
func (s *LightChainstoneeum) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *LightChainstoneeum) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *LightChainstoneeum) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// Chainstonebase is the address that mining rewards will be send to
func (s *LightDummyAPI) Chainstonebase() (common.Address, error) {
	return common.Address{}, fmt.Errorf("mining is not supported in light mode")
}

// Coinbase is the address that mining rewards will be send to (alias for Chainstonebase)
...
// APIs returns the collection of RPC services the chainstone package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightChainstoneeum) APIs() []rpc.API {
...
func (s *LightChainstoneeum) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightChainstoneeum) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightChainstoneeum) TxPool() *light.TxPool              { return s.txPool }
func (s *LightChainstoneeum) Engine() consensus.Engine           { return s.engine }
func (s *LightChainstoneeum) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightChainstoneeum) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightChainstoneeum) EventMux() *event.TypeMux           { return s.eventMux }
func (s *LightChainstoneeum) Merger() *consensus.Merger          { return s.merger }

// Protocols returns all the currently configured network protocols to start.
func (s *LightChainstoneeum) Protocols() []p2p.Protocol {
...
// light chainstone protocol implementation.
func (s *LightChainstoneeum) Start() error {
...
func (s *LightChainstoneeum) Stop() error {
	close(s.closeCh)
	s.serverPool.Stop()
	s.peers.close()
	s.reqDist.close()
	...
	s.lesDb.Close()
	s.wg.Wait()
	log.Info("Light chainstone stopped")
	return nil
}
