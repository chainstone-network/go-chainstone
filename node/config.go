...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gchain is "gchain". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gchain", keep that.
	if name == "gchain" || name == "gchain-testnet" {
		name = "Gchain"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gchain" instances.
var isOldGchainResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gchain 1.4 are used if they exist.
	if warn, isOld := isOldGchainResource[path]; isOld {
		oldpath := ""
		if c.name() == "gchain" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGchainResourceWarning, "Using deprecated resource file %s, please move this file to the 'gchain' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-chainstone-keystore")
		isEphemeral = true
	}
