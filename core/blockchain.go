...
// Package core implements the Chainstoneeum consensus protocol.
package core
...
import (
	...
	"github.com/chainstone-network/go-chainstone/common/prque"
	...
	"github.com/chainstone-network/go-chainstone/internal/syncx"
...
// available in the database. It initialises the default Chainstoneeum Validator
// and Processor.
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Gchain is initialized with
...
// it's not checked in the Gchain side anymore.
...
// sync, Gchain will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Gchain with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Gchain to a new version which supports transaction
