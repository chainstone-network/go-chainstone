...
// Verify that Client implements the chainstone interfaces.
var (
	_ = chainstone.ChainReader(&Client{})
	_ = chainstone.TransactionReader(&Client{})
	_ = chainstone.ChainStateReader(&Client{})
	_ = chainstone.ChainSyncReader(&Client{})
	_ = chainstone.ContractCaller(&Client{})
	_ = chainstone.GasEstimator(&Client{})
	_ = chainstone.GasPricer(&Client{})
	_ = chainstone.LogFilterer(&Client{})
	_ = chainstone.PendingStateReader(&Client{})
	// _ = chainstone.PendingStateEventer(&Client{})
	_ = chainstone.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Chainstoneeum Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new chainstone service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == chainstone.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != chainstone.NotFound {
		t.Fatal("error should be chainstone.NotFound")
	}
}
