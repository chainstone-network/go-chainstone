...
// Package gchainclient provides an RPC client for gchain-specific APIs.
package gchainclient
...
// Client is a wrapper around rpc.Client that implements gchain-specific functionality.
//
// If you want to use the standardized Chainstoneeum RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg chainstone.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg chainstone.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gchain node.
...
// MemStats retrieves the current memory stats from a gchain node.
...
// GetNodeInfo retrieves the node info of a gchain node.
