...
// ChainstoneeumAPI provides an API to access Chainstoneeum related information.
type ChainstoneeumAPI struct {
	b Backend
}

// NewChainstoneeumAPI creates a new Chainstoneeum protocol API.
func NewChainstoneeumAPI(b Backend) *ChainstoneeumAPI {
	return &ChainstoneeumAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *ChainstoneeumAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *ChainstoneeumAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *ChainstoneeumAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *ChainstoneeumAPI) Syncing() (interface{}, error) {
...
// ChainstoneeumAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type ChainstoneeumAccountAPI struct {
	am *accounts.Manager
}

// NewChainstoneeumAccountAPI creates a new ChainstoneeumAccountAPI.
func NewChainstoneeumAccountAPI(am *accounts.Manager) *ChainstoneeumAccountAPI {
	return &ChainstoneeumAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *ChainstoneeumAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Chainstoneeum ECDSA signature for:
// keccak256("\x19Chainstoneeum Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19Chainstoneeum Signed Message:\n"${message length}${message})
...
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Chainstoneeum signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gchain.
...
// BlockChainAPI provides an API to access Chainstoneeum blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Chainstoneeum blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Chainstoneeum chain config.
//
...
// DebugAPI is the collection of Chainstoneeum APIs exposed over the debugging
// namespace.
...
// Version returns the current chainstone protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Chainstone)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f chainstone) exceeds the configured cap (%.2f chainstone)", feeFloat, cap)
	}
	return nil
}
