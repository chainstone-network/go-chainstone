...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gchain.LightScryptN, Gchain.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.chainstone", new NodeConfig());
			node.start();
			...
			ChainstoneeumClient ec = node.getChainstoneeumClient();
	...
	// signatures too. Regression test for go-chainstone issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/chainstone/gchaintest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gchain and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.chainstone", "github.com/chainstone-network/go-chainstone/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gchain.aar"), "gchain.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "chainstone", "gchaintest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.chainstone.gchaintest"
