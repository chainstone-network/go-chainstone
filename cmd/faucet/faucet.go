// Copyright 2017 The go-chainstone Authors
...
// faucet is an Chainstone faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Chainstoneeum protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Chainstones to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Chainstoneeum and the faucet")
...
var (
	chainstone = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Chainstones", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Chainstoneeum address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Chainstoneeum light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Chainstoneeum protocol stack
	client *ethclient.Client   // Client connection to the Chainstoneeum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Chainstoneeum light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Chainstoneeum service: %w", err)
	}
...
// close terminates the Chainstoneeum connection and tears down the faucet.
...
// apiHandler handles requests for Chainstone grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(wsconn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, chainstone),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Chainstoneeum address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), chainstone)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, chainstone)
			peers := f.stack.Server().PeerCount()
...
// the uniqueness identifier (user id/username), username, avatar URL and Chainstoneeum address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Chainstoneeum address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Chainstoneeum address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Chainstoneeum address to fund on
// success.
...
// returning the username, avatar URL and Chainstoneeum address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Chainstoneeum address and profile URL.
	//
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Chainstoneeum address found to fund. Please check the post URL and verify that it can be viewed publicly.")
	}
	var avatar string
...
// authNoAuth tries to interpret a faucet request as a plain Chainstoneeum address,
