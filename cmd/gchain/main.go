...
// gchain is the official command-line client for Chainstoneeum.
package main
...
import (
	...
	_ "github.com/chainstone-network/go-chainstone/eth/tracers/js"
	_ "github.com/chainstone-network/go-chainstone/eth/tracers/native"

	"github.com/urfave/cli/v2"
)

const (
	clientIdentifier = "gchain" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-chainstone command line interface")
...
func init() {
	// Initialize the CLI app and start Gchain
	app.Action = gchain
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2022 The go-chainstone Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.IsSet(utils.RopstenFlag.Name):
		log.Info("Starting Gchain on Ropsten testnet...")

	case ctx.IsSet(utils.RinkebyFlag.Name):
		log.Info("Starting Gchain on Rinkeby testnet...")

	case ctx.IsSet(utils.GoerliFlag.Name):
		log.Info("Starting Gchain on Görli testnet...")

	case ctx.IsSet(utils.SepoliaFlag.Name):
		log.Info("Starting Gchain on Sepolia testnet...")

	case ctx.IsSet(utils.KilnFlag.Name):
		log.Info("Starting Gchain on Kiln testnet...")

	case ctx.IsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Gchain in ephemeral dev mode...")
		log.Warn(`You are running Gchain in --dev mode. Please note the following:
...
`)
	...
	case !ctx.IsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Gchain on Chainstoneeum mainnet...")
	}
...
// gchain is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gchain(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend, isConsole bool) {
	...
	// Create a client to interact with local gchain node.
	...
	if ctx.Bool(utils.MiningEnabledFlag.Name) || ctx.Bool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Chainstoneeum node is running
		if ctx.String(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("Chainstoneeum service not running")
		}
