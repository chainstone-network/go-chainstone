...
func (g *gchainrpc) killAndWait() {
	g.gchain.Kill()
	g.gchain.WaitExit()
}

func (g *gchainrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gchain.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gchainrpc) addPeer(peer *gchainrpc) {
	g.gchain.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gchain.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gchain.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gchain.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gchain.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gchainrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func startGchainWithIpc(t *testing.T, name string, args ...string) *gchainrpc {
	ipcName := fmt.Sprintf("gchain-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	// We can't know exactly how long gchain will take to start, so we try 10
...
func initGchain(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gchain: %v ", args)
	g := runGchain(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gchainrpc {
	datadir := initGchain(t)
	t.Logf("Importing keys to gchain")
	runGchain(t, "account", "import", "--datadir", datadir, "--password", "./testdata/password.txt", "--lightkdf", "./testdata/key.prv").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGchainWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gchainrpc {
	datadir := initGchain(t)
	return startGchainWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
