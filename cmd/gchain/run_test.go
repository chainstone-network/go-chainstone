...
func init() {
	// Run the app if we've been exec'd as "gchain-test" in runGchain.
	reexec.Register("gchain-test", func() {
...
// spawns gchain with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGchain(t *testing.T, args ...string) *testgchain {
	tt := &testgchain{}
	...
	for i, arg := range args {
		...
		case "--miner.chainstonebase":
			if i < len(args)-1 {
				tt.Chainstonebase = args[i+1]
			}
		}
	}
	if tt.Datadir == "" {
	...
	// Boot "gchain". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("gchain-test", args...)
