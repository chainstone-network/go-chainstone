...
func loadConfig(file string, cfg *gchainConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gchain.ipc"
	return cfg
}

// makeConfigNode loads gchain configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gchainConfig) {
	// Load defaults.
	cfg := gchainConfig{
...
// makeFullNode loads gchain configuration and creates the Chainstoneeum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gchain db freezer-migrate`.")
		}
	}
	...
	// Add the Chainstoneeum Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gchainConfig) {
