...
func TestAccountListEmpty(t *testing.T) {
	gchain := runGchain(t, "account", "list")
	gchain.ExpectExit()
}
...
`
	...
	{
		gchain := runGchain(t, "account", "list", "--datadir", datadir)
		gchain.Expect(want)
		gchain.ExpectExit()
	}
	{
		gchain := runGchain(t, "--datadir", datadir, "account", "list")
		gchain.Expect(want)
		gchain.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gchain := runGchain(t, "account", "new", "--lightkdf")
	defer gchain.ExpectExit()
	gchain.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gchain := runGchain(t, "account", "-h")
	gchain.WaitExit()
	if have, want := gchain.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gchain = runGchain(t, "account", "import", "-h")
	gchain.WaitExit()
	if have, want := gchain.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gchain := runGchain(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gchain.ExpectExit()
	gchain.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gchain := runGchain(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gchain := runGchain(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gchain.ExpectExit()
	gchain.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gchain.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gchain := runMinimalGchain(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gchain.StderrText(), m) {
...
// https://github.com/chainstone-network/go-chainstone/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gchain.SetTemplateFunc("keypath", func(file string) string {
