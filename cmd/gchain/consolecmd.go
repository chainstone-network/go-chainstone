...
import (
	...
	"github.com/chainstone-network/go-chainstone/console"
...
var (
	...
	consoleCommand = &cli.Command{
		...
		Description: `
The Gchain console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gchain.chainstone.org/docs/interface/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gchain.chainstone.org/docs/interface/javascript-console.
This command allows to open a console on a running gchain node.`,
	}
...
JavaScript API. See https://gchain.chainstone.org/docs/interface/javascript-console`,
	}
)

// localConsole starts a new gchain node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		return fmt.Errorf("Failed to attach to the inproc gchain: %v", err)
	}
...
// remoteConsole will connect to a remote gchain instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gchain: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new gchain node, attaches an ephemeral JavaScript
...
gchain --exec "%s" console`, b.String())
	return nil
}

// dialRPC returns a RPC client which connects to the given endpoint.
// The check for empty endpoint implements the defaulting logic
// for "gchain attach" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gchain < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
