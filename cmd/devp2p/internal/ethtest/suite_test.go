...
func TestEthSuite(t *testing.T) {
	gchain, err := runGchain()
	if err != nil {
		t.Fatalf("could not run gchain: %v", err)
	}
	defer gchain.Close()

	suite, err := NewSuite(gchain.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
...
// runGchain creates and starts a gchain node
func runGchain() (*node.Node, error) {
...
func setupGchain(stack *node.Node) error {
