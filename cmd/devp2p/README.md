...
Use `devp2p enrdump <base64>` to verify and display an Chainstoneeum Node Record.
...
1. initialize the gchain node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gchain with the following flags:
```
gchain --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the gchain node:
 ```
...
To run the eth66 protocol test suite, initialize a gchain node as described above and run the following command,
replacing `<enode>` with the enode of the gchain node:

 ```
 devp2p rlpx eth66-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```

[eth]: https://github.com/chainstone-network/devp2p/blob/master/caps/eth.md
[dns-tutorial]: https://gchain.chainstone.org/docs/developers/dns-discovery-setup
[discv4]: https://github.com/chainstone-network/devp2p/tree/master/discv4.md
[discv5]: https://github.com/chainstone-network/devp2p/tree/master/discv5/discv5.md
