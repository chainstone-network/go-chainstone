...
var nodeDockerfile = `
FROM chainstone/client-go:latest
...
{{end}}
RUN \
  echo 'gchain --cache 512 init /genesis.json' > gchain.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.chainstone/keystore/ && cp /signer.json /root/.chainstone/keystore/' >> gchain.sh && \{{end}}
	echo $'exec gchain --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Chainstonebase}}--miner.chainstonebase {{.Chainstonebase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gchain.sh

ENTRYPOINT ["/bin/sh", "gchain.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Chainstoneeum node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.chainstone{{if .Ethashdir}}
...
// deployNode deploys a new Chainstoneeum node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.chainstonebase == "" {
		kind = "bootnode"
...
func (info *nodeInfos) Report() map[string]string {
	...
	if info.gasTarget > 0 {
		...
		if info.chainstonebase != "" {
			// Ethash proof-of-work miner
			report["Ethash directory"] = info.ethashdir
			report["Miner account"] = info.chainstonebase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gchain --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
