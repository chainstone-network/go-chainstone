// Copyright 2015 The go-chainstone Authors
...
// Package utils contains internal helper functions for go-chainstone commands.
package utils
...
import (
	...
	ethcatalyst "github.com/chainstone-network/go-chainstone/eth/catalyst"
	...
	"github.com/chainstone-network/go-chainstone/eth/tracers"
	"github.com/chainstone-network/go-chainstone/ethdb"
	"github.com/chainstone-network/go-chainstone/ethdb/remotedb"
	"github.com/chainstone-network/go-chainstone/ethstats"
	"github.com/chainstone-network/go-chainstone/graphql"
	...
	lescatalyst "github.com/chainstone-network/go-chainstone/les/catalyst"
	"github.com/chainstone-network/go-chainstone/log"
	"github.com/chainstone-network/go-chainstone/metrics"
	"github.com/chainstone-network/go-chainstone/metrics/exp"
	"github.com/chainstone-network/go-chainstone/metrics/influxdb"
...
var (
	...
	MinerChainstonebaseFlag = &cli.StringFlag{
		Name:     "miner.chainstonebase",
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in chainstone) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "Gchain will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older Gchain configurations storing the
...
// for Gchain and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gchain account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// setChainstonebase retrieves the chainstonebase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setChainstonebase(ctx *cli.Context, ks *keystore.KeyStore, cfg *ethconfig.Config) {
	// Extract the current chainstonebase
	var chainstonebase string
	if ctx.IsSet(MinerChainstonebaseFlag.Name) {
		chainstonebase = ctx.String(MinerChainstonebaseFlag.Name)
	}
	// Convert the chainstonebase into an address and configure it
	if chainstonebase != "" {
		if ks != nil {
			account, err := MakeAddress(ks, chainstonebase)
			if err != nil {
				Fatalf("Invalid miner chainstonebase: %v", err)
			}
			cfg.Miner.Chainstonebase = account.Address
		} else {
			Fatalf("No chainstonebase configured")
		}
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.Bool(DeveloperFlag.Name):
		...
		// setChainstonebase has been called above, configuring the miner address from command line flags.
		if cfg.Miner.Chainstonebase != (common.Address{}) {
			developer = accounts.Account{Address: cfg.Miner.Chainstonebase}
...
// RegisterEthService adds an Chainstoneeum client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Chainstoneeum) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Chainstoneeum service: %v", err)
		}
...
// RegisterEthStatsService configures the Chainstoneeum Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Chainstoneeum Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gchain.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gchain.", tagsMap)
		}
